...
class Player(BaseBot):
    ...
    def get_p_win(self, current_state, game_info):
        ...
        # --- FIXED 3-CARD BLINDSPOT ---
        opp_card_count = 2
        # If the auction is over and we only have 2 cards, the opponent MUST have 3.
        if current_state.street in ['flop', 'turn', 'river'] and len(current_state.my_hand) == 2:
            opp_card_count = 3
            
        opp_needed = opp_card_count - len(opp_locked)
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if street == 'auction':
            ...
            # FIXED: Hard cap at 400 to prevent Kamikaze Auction Traps
            return ActionBid(int(max(0, min(bid, my_chips, 400))))
        ...
        if cost > pot * 0.8:
            adjusted_p_win -= 0.15  # Massive Overbet
        elif cost > pot * 0.4:  
            adjusted_p_win -= 0.08  # Standard Value Bet

        # FIXED: Dynamic Preflop Defense (Don't fold playable hands!)
        if not current_state.board and cost > 0:
            required_equity = 0.55 if cost > 100 else 0.49
