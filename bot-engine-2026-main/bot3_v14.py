...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        # --- 1. THE INFORMATION AUCTION ---
        if street == 'auction':
            # Polarized mixed strategy to crush predictable bots. 
            if p_win > 0.70:
                # Value bid: Deny information when we have strong hands.
                bid = int(pot * random.uniform(1.2, 1.8))
            elif p_win > 0.45:
                # Marginal: Pay cheaply for the opponent's card.
                bid = int(pot * random.uniform(0.1, 0.4))
            else:
                # Polarized Bluff: Confuse their tracking algorithms.
                bid = int(pot * 1.5) if random.random() < 0.15 else random.randint(0, int(pot * 0.15))

            return ActionBid(int(max(0, min(bid, my_chips))))

        # --- 2. LOCAL BEST RESPONSE (BAYESIAN ADJUSTMENT) ---
        pot_odds = cost / (pot + cost) if cost > 0 else 0
        
        # If an opponent makes a large bet, they rarely have garbage.
        # We shrink our perceived win probability to mimic folding to their strong range.
        adjusted_p_win = p_win
        if cost > pot * 0.4:  
            adjusted_p_win -= 0.10  

        # Strict Preflop Defense
        if not current_state.board and cost > 0 and adjusted_p_win < 0.55:
        ...
        if current_state.can_act(ActionRaise):
            if p_win > 0.75:
                # Pure Value (Nuts)
                target_raise_to = current_state.my_wager + cost + int(pot * random.uniform(0.7, 1.0))
                min_r, max_r = current_state.raise_bounds
                rais = int(max(min_r, min(target_raise_to, max_r)))
                return ActionRaise(rais)
            
            elif p_win > 0.60:
                # Thin Value
                target_raise_to = current_state.my_wager + cost + int(pot * 0.4)
                min_r, max_r = current_state.raise_bounds
                rais = int(max(min_r, min(target_raise_to, max_r)))
                return ActionRaise(rais)

            elif p_win < 0.45 and cost == 0 and random.random() < 0.30:
                # Small-Ball C-Bet Bluff (The bot3_v12 killer). 
                # When they check, we bet 33% pot. It forces them to fold all marginal hands.
                target_raise_to = current_state.my_wager + cost + int(pot * 0.33)
                min_r, max_r = current_state.raise_bounds
                rais = int(max(min_r, min(target_raise_to, max_r)))
                return ActionRaise(rais)
        ...
        if current_state.can_act(ActionCall):
            # If we know their card, math is highly precise.
            safety_margin = 0.02 if current_state.opp_revealed_cards else 0.08
            
            # Use the mathematically adjusted LBR win probability against big bets
            if adjusted_p_win > (pot_odds + safety_margin):
                return ActionCall()
            else:
                return ActionFold()
